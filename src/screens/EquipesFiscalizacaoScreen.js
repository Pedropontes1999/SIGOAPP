...
export default function EquipesFiscalizacaoScreen({ route, navigation }) {
  ...
  function registrarConferencia(sigla, presencas) {
    setConferencias(prev => ({ ...prev, [sigla]: { ausente: false, presencas } }));
  }
  ...
  function abrirEquipe(equipe) {
    navigation.navigate('ConfirmacaoEquipe', {
      ...
      onConfirmar: (presencas) => registrarConferencia(equipe.sigla, presencas),
    });
  }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={[styles.body, { backgroundColor: colors.bg }]}>
        ...
        {/* Supervisor de campo presente na obra (pode adicionar vários) */}
        <View style={[styles.supCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          {supervisores.length === 0 ? (
            <Text style={[styles.supEmpty, { color: colors.equipeSub }]}>
              Toque em + para adicionar o(s) supervisor(es) presente(s) na obra.
            </Text>
          ) : (
      ...
      {/* Avançar só libera quando todas as equipes estiverem conferidas */}
      <View style={[styles.footer, { backgroundColor: colors.footerBg, borderTopColor: colors.footerBorder }]}>
        <TouchableOpacity
          style={[styles.footerBtn, !todasFeitas && styles.footerBtnDisabled]}
          onPress={avancar}
          disabled={!todasFeitas}
          activeOpacity={0.85}
        >
          <Text style={styles.footerBtnText}>
            {todasFeitas ? 'Avançar' : `Conferir todas (${feitas}/${total})`}
          </Text>
        </TouchableOpacity>
      </View>
